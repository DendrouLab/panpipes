...
if args.customgenesfile is not None:
    if os.path.exists(args.customgenesfile):
        cat_dic = {}
        customgenes = pd.read_csv(args.customgenesfile, sep='\t')
        custom_cat = list(set(customgenes['gene_group'].tolist()))
        for cc in custom_cat:
            cat_dic[cc] = customgenes.loc[customgenes["gene_group"] == cc,"gene_name"].tolist()
