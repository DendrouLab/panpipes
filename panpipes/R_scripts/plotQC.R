...
for (sc in rna_source_facet) {
  ...
  if (all(c("total_counts", "n_genes_by_counts") %in% colnames(rna_data_plot))) {
    ...
    ggsave(g,
      filename = file.path(outpath, paste0("scatter_", sc, "_rna-nUMI_v_rna-genes.png")), type = "cairo",
  ...
  if (all(c("log1p_total_counts", "log1p_n_genes_by_counts") %in% colnames(rna_data_plot))) {
    ...
    ggsave(g,
      filename = file.path(outpath, paste0("scatter_", sc, "_rna-log1p_nUMI_v_rna-log1p_genes.png")), type = "cairo",
      width = 3 * ncols, height = 3 * nrows, dpi = 200
    )
  }
  if (all(c("total_counts", "pct_counts_mt") %in% colnames(rna_data_plot))) {
    ...
    ggsave(g,
      filename = file.path(outpath, paste0("scatter_", sc, "_rna-nUMI_v_rna-pct_mt.png")), type = "cairo",
      width = 3 * ncols, height = 3 * nrows, dpi = 200
    )
  }
  if (all(c("n_genes_by_counts", "doublet_scores", "total_counts") %in% colnames(rna_data_plot))) {
    cat("Plotting scatter plot of n_genes_by_counts and doublet_scores\n")
    g <- do_scatter_plot(rna_data_plot,
      x = "n_genes_by_counts", y = "doublet_scores",
      hue = "total_counts", facet = sc
    )
    ggsave(g,
      filename = file.path(outpath, paste0("scatter_", sc, "_rna-genes_rna-doublet_scores_rna-numi.png")),
...
if (!is.null(opt$prot_qc_metrics)) {
  ...
  for (sc in prot_source_facet) {
    ...
    if (all(c("total_counts", "n_prot_by_counts") %in% colnames(prot_data_plot))) {
      cat("Plotting scatter plot of total_counts and n_prot_by_counts\n")
      g <- do_scatter_plot(prot_data_plot, x = "total_counts", y = "n_prot_by_counts", facet = sc)
      ggsave(g,
        filename = file.path(outpath, paste0("scatter_", sc, "_prot-nUMI_v_prot-prot.png")), type = "cairo",
        width = 3 * ncols, height = 3 * nrows, dpi = 200
      )
    }
    if (all(c("log1p_total_counts", "log1p_n_prot_by_counts") %in% colnames(prot_data_plot))) {
      cat("Plotting scatter plot of log1p_total_counts and log1p_n_prot_by_counts\n")
      g <- do_scatter_plot(prot_data_plot, x = "log1p_total_counts", y = "log1p_n_prot_by_counts", facet = sc)
      ggsave(g,
        filename = file.path(outpath, paste0("scatter_", sc, "_prot-log1p_nUMI_v_prot-log1p_prot.png")), type = "cairo",
        width = 3 * ncols, height = 3 * nrows, dpi = 200
      )
    }
    if (all(c("total_counts", "pct_counts_isotype") %in% colnames(prot_data_plot))) {
      cat("Plotting scatter plot of total_counts and pct_counts_isotype\n")
      g <- do_scatter_plot(prot_data_plot, x = "total_counts", y = "pct_counts_isotype", facet = sc)
      ggsave(g,
        filename = file.path(outpath, paste0("scatter_", sc, "_prot-nUMI_v_prot-pct_isotype.png")), type = "cairo",
        width = 3 * ncols, height = 3 * nrows, dpi = 200
      )
    }
    if (all(c("total_counts", "total_counts_isotype") %in% colnames(prot_data_plot))) {
      cat("Plotting scatter plot of total_counts and total_counts_isotype\n")
      g <- do_scatter_plot(prot_data_plot, x = "total_counts", y = "total_counts_isotype", facet = sc)
      ggsave(g,
        filename = file.path(outpath, paste0("scatter_", sc, "_prot-nUMI_v_prot-total_counts_isotype.png")), type = "cairo",
        width = 3 * ncols, height = 3 * nrows, dpi = 200
      )
    }
    if (all(c("log1p_total_counts", "log1p_total_counts_isotype", "isotype_exclude") %in% colnames(prot_data_plot))) {
      cat("Plotting scatter plot of log1p_total_counts and log1p_total_counts_isotype\n")
      g <- do_scatter_plot(prot_data_plot, x = "log1p_total_counts", y = "log1p_total_counts_isotype", hue = "isotype_exclude", facet = sc)
      ggsave(g,
        filename = file.path(outpath, paste0("scatter_", sc, "_prot-log1p_nUMI_v_prot-log1p_total_counts_isotype.png")), type = "cairo",
        width = 3 * ncols, height = 3 * nrows, dpi = 200
      )
    }
  }
...
if (!is.null(opt$prot_qc_metrics)) {
  ...
  for (sc in source_facet) {
    ...
    if (all(c("rna.total_counts", "prot.total_counts") %in% colnames(data_plot))) {
      cat("Plotting scatter plot of rna.total_counts and prot.total_counts\n")
      g <- do_scatter_plot(data_plot, x = "rna.total_counts", y = "prot.total_counts", facet = sc)
      ggsave(g,
        filename = file.path(outpath, paste0("scatter_", sc, "-nUMI_v_rna-nUMI.png")), type = "cairo",
        width = 3 * ncols, height = 3 * nrows, dpi = 200
      )
    }
    if (all(c("rna.log1p_total_counts", "prot.log1p_total_counts") %in% colnames(data_plot))) {
      cat("Plotting scatter plot of rna.log1p_total_counts and prot.log1p_total_counts\n")
      g <- do_scatter_plot(data_plot, x = "rna.log1p_total_counts", y = "prot.log1p_total_counts", facet = sc)
      ggsave(g,
        filename = file.path(outpath, paste0("scatter_", sc, "-log1p_nUMI_v_rna-log1p_nUMI.png")), type = "cairo",
        width = 3 * ncols, height = 3 * nrows, dpi = 200
      )
    }
    if (all(c("rna.total_counts", "prot.total_counts_isotype") %in% colnames(data_plot))) {
      cat("Plotting scatter plot of rna.total_counts and prot.total_counts_isotype\n")
      g <- do_scatter_plot(data_plot, x = "rna.total_counts", y = "prot.total_counts_isotype", facet = sc)
      ggsave(g,
        filename = file.path(outpath, paste0("scatter_", sc, "-nUMI_v_prot-counts_isotype.png")), type = "cairo",
        width = 3 * ncols, height = 3 * nrows, dpi = 200
      )
    }
    if (all(c("rna.log1p_total_counts", "prot.log1p_total_counts_isotype") %in% colnames(data_plot))) {
      cat("Plotting scatter plot of rna.log1p_total_counts and prot.log1p_total_counts_isotype\n")
      g <- do_scatter_plot(data_plot, x = "rna.log1p_total_counts", y = "prot.log1p_total_counts_isotype", facet = sc)
      ggsave(g,
        filename = file.path(outpath, paste0("scatter_", sc, "-log1p_nUMI_v_prot-log1p_counts_isotype.png")), type = "cairo",
        width = 3 * ncols, height = 3 * nrows, dpi = 200
      )
    }
    if (all(c("rna.doublet_scores", "prot.log1p_total_counts") %in% colnames(data_plot))) {
      cat("Plotting scatter plot of rna.doublet_scores and prot.log1p_total_counts\n")
      g <- do_scatter_plot(data_plot, x = "rna.doublet_scores", y = "prot.log1p_total_counts", facet = sc)
      ggsave(g,
        filename = file.path(outpath, paste0("scatter_", sc, "-doublet_scores_v_prot-log1p_nUMI.png")), type = "cairo",
        width = 3 * ncols, height = 3 * nrows, dpi = 200
      )
    }
  }
...
if (opt$prefilter) {
  ...
  cat("Producing files with final counts for cells after filtering\n")
  ...
  g <- baseline %>%
    ...
    theme_bw() +
    theme(
      axis.text.x = element_text(size = 8, angle = 45, hjust = 1.05, vjust = 0.95),
      axis.text.y = element_text(size = 13)
    )
