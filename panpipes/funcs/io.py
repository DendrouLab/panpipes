...
from scanpy import read_10x_h5, read_10x_mtx, read_csv, read_h5ad, read_hdf, read_text
from scipy.io import mmwrite

from .processing import intersection


def update_cellranger_col(
    path: str | Path, raw: bool = False, method: str = "count", sample_id: str = ""
):
    if method == "count":
    ...
    if not os.path.exists(os.path.join(path, append_str)):
...
def gen_load_anndata_jobs(
    caf: pd.DataFrame,
    load_raw: bool = False,
    mode_dictionary: Optional[dict] = None,
    load_prot_from_raw: bool = False,
):
    if mode_dictionary is None:
        mode_dictionary = {}  # not the best, dirty hack to make it compatible with code
    ...
    for nn in range(0, caf.shape[0]):
        if pd.isna(caf["rna_path"][nn]):
            ...
            rna_path, rna_filetype = update_cellranger_col(
                caf["rna_path"][nn], raw=load_raw, method="count"
            )
        ...
        if "prot_path" in caf.columns and mode_dictionary["prot"]:
            ...
            if pd.isna(caf["prot_path"][nn]):
                ...
                prot_path, prot_filetype = update_cellranger_col(
                    caf["prot_path"][nn], raw=load_prot_from_raw
                )
        ...
        # load tcr_path
        if (
            "tcr_path" in caf.columns
            and mode_dictionary["tcr"]
            and pd.notna(caf["tcr_path"][nn])
        ):
        ...
        # load bcr_path
        if (
            "bcr_path" in caf.columns
            and mode_dictionary["bcr"]
            and pd.notna(caf["bcr_path"][nn])
        ):
        ...
        if "atac_path" in caf.columns and mode_dictionary["atac"]:
            if caf.shape[0] > 1:
                sys.exit(
                    "You can only submit one atac/multiome file at a time. To aggregate, see cellranger aggr."
                )
            if caf["atac_filetype"][nn] == "cellranger":
                atac_path, atac_filetype = update_cellranger_col(
                    caf["atac_path"][nn], raw=load_raw, method="count"
                )
            else:
            ...
            if "fragments_file" in caf.columns and pd.notna(caf["fragments_file"][nn]):
                ...
                fragments_file = None
            if "per_barcode_metrics_file" in caf.columns and pd.notna(
                caf["per_barcode_metrics_file"][nn]
            ):
                per_barcode_metrics_file = caf["per_barcode_metrics_file"][nn]
            else:
                per_barcode_metrics_file = None
            if "peak_annotation_file" in caf.columns and pd.notna(
                caf["peak_annotation_file"][nn]
            ):
...
def gen_load_spatial_jobs(
    caf: pd.DataFrame, mode_dictionary: Optional[dict] = None, load_raw: bool = True
):
    """
    ...
    for nn in range(0, caf.shape[0]):
        ...
        # ToDo: cell_mtd_path is not used in this function below
        if "barcode_mtd_path" in caf.columns:
...
def read_anndata(
    ...
    if fname is None:
        sys.exit("anndata file not found; `fname` has not been passed/")

    # check fname file exists
    try:
        os.path.isfile(fname)
    except FileNotFoundError:
        sys.exit(f"anndata file not found: {fname}")
    ...
    if use_muon is False:
        ...
        if modality == "rna":
            ...
            sys.exit(
                f"modality {modality} not found, must be one of 'all', 'rna', 'prot','atac', 'rep' "
            )
...
def write_obs(
    mdata: AnnData,
    output_prefix: str = "./",
    output_suffix: str = "_filtered_cell_metadata.tsv",
):
    metafile = mdata.obs
...
def check_submission_file(caf: pd.DataFrame):
    """
    Fixing the column names in the input data frame in case formatting is off.
    """
    old_columns = caf.columns
    caf.rename(
        columns={col_name: col_name.strip() for col_name in caf.columns}, inplace=True
    )
    caf.replace("^ +| +$", "", regex=True, inplace=True)
    logging.info(
        f"Formatted columns to remove spaces. Changes: {old_columns} -> {caf.columns}",
    )

    if "filetype" in caf.columns or "path" in caf.columns:
        raise ValueError(
            "you appear to be using the old notation for the sample submission file, \
...
def check_filetype(
    ...
    if filetype == "cellranger":
        if os.path.exists(path + "/matrix.mtx.gz"):
            ...
            sys.exit(
                "cellranger matrix file not found, have you specified the path correctly"
            )
    else:
        ftype_checks = {
            "h5ad": r".h5ad$",
        ...
        if filetype not in ftype_checks.keys():
            sys.exit(
                f"unknown filetype {filetype}, please specify one of: {list(ftype_checks.keys())}"
            )
        elif re.search(ftype_checks[filetype], path) is None:
            sys.exit(
                "filetype does not match file suffix, check it is specified correctly \
                    (with %s as suffix (gzipped files ok for csv and tsv))"
                % ftype_checks[filetype]
            )
...
def read_scirpy(
    ...
    if filetype == "cellranger_vdj":
        ...
        sys.exit(
            f"filetype {filetype} not valid, must be one of 'cellranger_vdj', 'tracer', 'bracer', 'airr'"
        )
    return vdj


def scp_read_10x_h5(
    filename: PathLike, library_keep=None, extended: bool = False, *args, **kwargs
) -> AnnData:
    """
...
def scp_read_10x_mtx(filename: PathLike, library_keep=None, *args, **kwargs) -> AnnData:
    ...
    """
    adata = read_10x_mtx(
        filename, gex_only=False, *args, **kwargs
    )  # need to leave gex as this is scanpy's code
...
def load_adata_in(
    path: str,
    filetype: str,
    gex_only: bool = True,  # ToDo: it not used anywhere in the function
    ...
    load_functions_dict = {
        "cellranger": lambda x: scp_read_10x_mtx(
            x, library_keep=library, var_names=var_names, cache=False
        ),
        "h5ad": read_h5ad,
        "csv_matrix": read_csv,
...
def update_intersecting_feature_names(adata1: AnnData, adata2: AnnData, prefix: str):
    # check for intersecting names
    if len(adata1.var_names) > 0 and len(adata2.var_names) > 0:
        intersect_features = intersection(
            adata1.var_names.tolist(), adata2.var_names.tolist()
        )
        if len(intersect_features) > 0:
            # this will be a problem downstream, so we prefix the
            warnings.warn(
                "intersecting gene ids and other modalities are problematic downstream, \
                prefixing  with %s"
                % prefix
            )
...
def load_mdata_from_multiple_files(all_files_dict):
    ...
    for nm, x in all_files_dict.items():
        ...
        if nm == "spatial":
            extra_args["gex_only"] = (
                True  # check this for techs other than merfish and visium H&E
            )
    ...
    if "rna" in data_dict.keys():
        ...
        for mod in data_dict.keys():
            if mod == "rna":
                ...
                data_dict[mod] = update_intersecting_feature_names(
                    adata1=data_dict["rna"], adata2=data_dict[mod], prefix=mod + "_"
                )
...
def write_10x_counts(adata: AnnData, path: str, layer=None):
    if layer is None:
        arr = adata.X
    else:
    ...
    with (
        open(os.path.join(path, "matrix.mtx"), "rb") as f_in,
        gzip.open(os.path.join(path, "matrix.mtx.gz"), "wb") as f_out,
    ):
    ...
    barcodes.to_csv(
        os.path.join(path, "barcodes.tsv.gz"), sep="\t", index=None, header=None
    )
    ...
    if "gene_ids" in adata.var.columns:
        ...
        if "prot_id" in features.columns:
            features = features.drop(columns=["gene_symbols"]).rename(
                columns={"prot_id": "gene_symbols"}
            )
    features = features[["gene_ids", "gene_symbols", "feature_types"]]
    features.to_csv(
        os.path.join(path, "features.tsv.gz"), sep="\t", index=None, header=None
    )
