...
from panpipes.funcs.io import gen_load_anndata_jobs
...
PARAMS["py_path"] = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "python_scripts"
)
PARAMS["r_path"] = os.path.join(os.path.dirname(os.path.dirname(__file__)), "R_scripts")
PARAMS["resources_path"] = os.path.join(
...
def aggregate_tenx_metrics_multi(outfile):
    ...
    log_msg = (
        "TASK: 'aggregate_tenx_metrics_multi'"
        + f" IN CASE OF ERROR, PLEASE REFER TO : '{outfile}' FOR MORE INFORMATION."
    )
...
def gen_load_filtered_anndata_jobs():
    ...
    return gen_load_anndata_jobs(
        ...
        load_raw=False,
        mode_dictionary=PARAMS["modalities"],
        load_prot_from_raw=PARAMS["load_prot_from_raw"],
    )
...
def load_mudatas(
    ...
    modality_dict = {
        k: True if path_dict[k] is not None else False
        for k, v in PARAMS["modalities"].items()
    }
    print(modality_dict)
    ...
    log_msg = (
        "TASK: 'load_mudatas'"
        + f" IN CASE OF ERROR, PLEASE REFER TO : '{logfile}' FOR MORE INFORMATION."
    )
    get_logger().info(log_msg)
    P.run(cmd, **job_kwargs)
...
@active_if(PARAMS["use_existing_h5mu"] is False)
@collate(load_mudatas, formatter(""), unfilt_file())
def concat_filtered_mudatas(infiles, outfile):
    # print(infiles)
    # print(outfile)
    infiles_str = ",".join(infiles)  # TODO: not used anywhere
...
if PARAMS["assess_background"] or (
    PARAMS["modalities"]["prot"] and "dsb" in PARAMS["normalisation_methods"]
):
...
@transform(
    load_bg_mudatas, regex("./tmp/(.*)_raw.h5(.*)"), r"./logs/1_\1_bg_downsampled.log"
)
...
@collate(load_bg_mudatas, formatter(""), bg_file())
...
@transform(
    load_mudatas, regex(r"./tmp/(.*).h5(.*)"), r"scrublet/\1_scrublet_scores.txt", r"\1"
)
...
def run_rna_qc(log_file, outfile, unfilt_file):
    ...
    log_msg = (
        "TASK: 'run_rna_qc'"
        + f" IN CASE OF ERROR, PLEASE REFER TO : '{log_file}' FOR MORE INFORMATION."
    )
    get_logger().info(log_msg)
